...
Task: verify that Claude Code can use Oracle Pro Extended and wait for the result.
...
oracle --dry-run summary --files-report --engine browser --model gpt-5.5-pro \
  ...
  --slug "claude smoke cart evaluation" \
  -p "Evaluate this tiny JavaScript cart module as a reviewer. Focus on correctness, mutation side effects, null handling, and whether the tests would catch regressions. This is a smoke test, but give a real, varied review response." \
